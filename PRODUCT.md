...
**Part 2 (built; sending still needs its first run with real Google accounts):** after the shifts happen, users record the real start and end times, nights slept at work, expenses and "Other" jobs. ShiftBoard works out the pay from each person's own rates, fills a monthly table in their own Google Sheet, and at month end, after the user has reviewed it, emails a PDF of the month to the company.
...
### Part 2 rules (confirmed and built)
...
- **Rates:** a change applies to every month not yet sent, for the whole month (a mid-month raise covers that month). Sent months never change. A reopened month keeps the rates it was sent with, unless the person switches it to their current rates.
- **The monthly report:** the summary first (name, month, year, total to pay (gross salary), expenses reimbursement), then a small hours summary for anyone who wants it, then one row per shift with a totals row. Rates never appear on it, and it uses plain hyphens, never long dashes.
- **Languages:** each person picks the app's language (English or Hebrew, right to left in Hebrew) on their device, and separately the report language (Hebrew by default, or English). A report and the sheet's tabs are always entirely one language, never mixed. In Hebrew the week starts on Sunday.
- **Currency:** Israeli shekel (₪).
- **Sending:** a review screen shows the recipient, the name, the totals and the PDF itself (every shift is in it); Send stays locked until every shift has times. The PDF is the month's report drawn by the app in the sheet's layout (A4 landscape pages, as images); the email's subject and text, which repeat both totals, are in the report language. Send emails the PDF from the user's Gmail to the company address, keeps a copy in a "ShiftBoard" folder in the user's Drive and marks the month sent. A sent month can be reopened, fixed and resent marked "Corrected"; the app flags a sent month whose calendar changed since.
- **Identity on the PDF:** the user's full name, which the user must confirm (with the company email) before the first send. It is written in the report's language: Hebrew letters on a Hebrew report, English letters on an English one. The app won't save, write to the sheet or send a name in the other script.
...
- **Permissions:** Google Calendar, Google Drive (only files the app creates) and Gmail send (asked for only at the first Send; it can't read mail).
...
- The app itself.
- The owner's current spreadsheet (a monthly table template and a pay rate sheet) and the Apps Script that calculates hours and pay exist but weren't shared; the owner asked for the table to be designed fresh, so the report's layout is the app's own.
...
3. Quiet until it matters: speak up for unsaved changes, missing times, a month left unsent, failures, and an expired sign-in, nothing else.
