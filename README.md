...
- **Dates and times** follow the app's language to start with — Hebrew shows 24-hour times and day-first dates, English shows AM/PM and month-first — and both can be changed under the gear, after which your choice stands whatever the language does afterwards. Two things sit outside it: the date and time boxes inside a day's panel are the phone's own pickers, drawn in the phone's language, which no website can override; and the sheet and the PDF always use `dd/mm/yyyy` and 24-hour times, because they're payroll records going to the company and shouldn't change shape from one coworker to the next.
...
- **Install it on a phone:** Android offers to add ShiftBoard to the home screen by itself; on an iPhone, tap Share → Add to Home Screen. It then opens like an app, without the browser bar. Once it has been opened on that phone it also works with no signal — the board, the Pay tab and your saved details all load — but reading and saving shifts is the calendar's job, so that part waits until there's a connection again. When that happens a note at the top says so in your language, instead of the app failing with "Couldn't load your shifts"; the note clears itself and the board catches up the moment the signal is back. Nothing is installed from a store and there's nothing to pay.
