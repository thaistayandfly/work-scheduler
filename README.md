...
- Sign-in uses Google Identity Services in the browser — no server. The access token (good for about an hour) is kept in `localStorage`, so reloading the page keeps you signed in. When the hour is up nothing is taken away: the board, the month and anything half-typed stay exactly where they are, and a note at the top — which follows you as you scroll — carries on with one tap, no consent screen. Open the app later with the hour already gone and it still shows the shifts it last read, with the same one tap. You don't have to sign in before working, either: tap a day, enter the times and save, and it's kept on the phone as a draft exactly as it would be with no signal — sign in when you're done and it offers to put them in the calendar. Only **Sign out** puts the welcome page back.
